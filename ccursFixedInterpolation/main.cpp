...
CMyGlut* CMyGlut::pInstance = nullptr;
...
double shooting(double x0, double x1, double y0, double y1, double h, bool shoot_derivaive = false) {
    double up(7), down(0.0);
    double fa, fb;

    do {
        fa = RungeKutta2time_calc(x0, x1, h, dvector_t{y0, up}, shoot_derivaive) - y1;
        fb = RungeKutta2time_calc(x0, x1, h, dvector_t{y0, down}, shoot_derivaive) - y1;
        up -= h;
        down += h;
    } while (fa * fb > 0);
    ...
    while(fabs(fc) > 1e-3) {
        ...
        fa = RungeKutta2time_calc(x0, x1, h, dvector_t{y0, up}, shoot_derivaive) - y1;
        fb = RungeKutta2time_calc(x0, x1, h, dvector_t{y0, down}, shoot_derivaive) - y1;
        fc = RungeKutta2time_calc(x0, x1, h, dvector_t{y0, c},shoot_derivaive) - y1;
        if(fa * fc < 0) {
            down = c;
        } else if (fc * fb < 0) {
            up = c;
...
vector<dpoint_t> getPoints(double x0, double x1, const dvector_t &y0, double &h) {
    ...
    do {
        ...
        res2.resize(n);
        double res_last = res.back().second[0];
        res2[0].second = y0;
        res2[0].first = x0;
        for (int i = 1; i < n; ++i) {
            res2[i].first = x0 + i * h;
...
int main(int argc, char** argv) {
    ...
    double y0 = 2, y1 = 7;
    const double h0 = 0.2;
    dvector_t y {y0, shooting(x0, x1, y0, y1, h0, false) + 0.05};

    double h = h0;
    vector<dpoint_t> p = getPoints(x0, x1, y, h);
    ofstream fout("solve.txt", ios_base::out);
    cout << "y(1)= " << p.back().second[0] << endl;
    for (auto& val : p) {
        fout << val.first << " " << val.second[0] << " "  << val.second[1] << endl;
    }
    fout.close();
    ...
    ifstream interpolation("solve.txt");
    pvector_t points;
    point_t in;
    dvector_t _y(p.size());
    for (size_t i = 0; i < p.size(); i++) {
        interpolation >> in.first >> in.second >> _y[i] ;
        points.emplace_back(in);
    }

    pvector_t print(points);
    for (point_t calc(print[0].first + 1, 0); calc.first < points[p.size() - 1].first; ++calc.first) {
        calc.second = newton(points, x0 + calc.first * h, p.size(), h0);
        print.emplace_back(calc);
    }
    std::sort(print.begin(), print.end(), [](point_t &left, point_t &right) { return left.first < right.first; });
    CMyGlut *test = CMyGlut::init(&argc, argv);
    const vector<pair<double, double> > results(p.size());
    test->displayFunc(print, results);
    return 0;
}
